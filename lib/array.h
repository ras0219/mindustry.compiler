...
__attribute__((nonnull)) __forceinline size_t array_size(const struct Array* arr, size_t sz) { return arr->sz / sz; }
...
__forceinline void array_concat(struct Array* arr, const struct Array* arr2) { array_push(arr, arr2->data, arr2->sz); }
...
__forceinline void* array_end(const struct Array* arr) { return arr->data ? (char*)arr->data + arr->sz : NULL; }
