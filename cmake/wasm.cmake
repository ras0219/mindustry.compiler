...
if(CMAKE_BUILD_TYPE STREQUAL "Release")
    set(CMAKE_CXX_CREATE_SHARED_LIBRARY
        "<CMAKE_CXX_COMPILER> <CMAKE_SHARED_LIBRARY_CXX_FLAGS> <LANGUAGE_COMPILE_FLAGS> <LINK_FLAGS> <CMAKE_SHARED_LIBRARY_CREATE_CXX_FLAGS> <SONAME_FLAG><TARGET_SONAME> -o <TARGET_BASE>.unopt.wasm <OBJECTS> <LINK_LIBRARIES>" "\"${WasmOpt_EXECUTABLE}\" -o <TARGET> -Oz <TARGET_BASE>.unopt.wasm")
endif()
    ...
    foreach(source IN LISTS arg_SOURCES)
        ...
        if(fn STREQUAL "cache.manifest.in")
            ...
            if(ext STREQUAL ".md")
                ...
                add_custom_command(
                    OUTPUT "${build_file}"
                    COMMAND "${Pandoc_EXECUTABLE}" "${afn}" -o "${build_file}"
                    DEPENDS "${afn}"
                )
...
function(add_wasmjs_target arg_target wasm_target output_file)
    ...
    add_custom_command(
        OUTPUT "${output_file}"
        COMMAND "${Wasm2JS_EXECUTABLE}" "${input_file}" --emscripten ">" "${output_file}"
        DEPENDS "${wasm_target}"
        VERBATIM
    )
